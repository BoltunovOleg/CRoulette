...
namespace ChatRoulette.Core.Session
{
    public class SessionController : INotifyPropertyChanged
    {
        ...
        public async Task<ChatConnectionResultEnum?> KeyDown(Key key)
        {
            ...
            var userId = await BrowserController.GetUserId();
            this.CurrentConnectionInfo.DateEnd = DateTime.Now;
            ...
            this.MakeScreenShoot(result, userId);
        ...
        private void MakeScreenShoot(ChatConnectionResultEnum folderType, string userId)
        {
            ...
            var fileName = Path.Combine(path, $"{userId}-{files.Length + 1}.jpg");
        ...
        private void MakeScreenShoot(string folder)
        {
            var image = this.BrowserController.GetBrowserScreenShot(new JpegBitmapEncoder());
            var path = this.GetCurrentSessionPathWithResult(folder);
            var files = Directory.GetFiles(path);
            var fileName = Path.Combine(path, $"{files.Length + 1}.jpg");
        ...
        private void StopSession()
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Произошла ошибка, возможно отчет не был отправлен", "Критическая ошибка");
                this.OnSessionEnd();
            }
        }
